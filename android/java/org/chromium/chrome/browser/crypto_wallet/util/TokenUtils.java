...
public class TokenUtils {
    ...
    public static void isCustomToken(
            @NonNull BlockchainRegistry blockchainRegistry,
            @NonNull NetworkInfo selectedNetwork,
            int coinType,
            @NonNull BlockchainToken token,
    ...
    /**
     * Checks if two tokens are equal. Two tokens are equal if: - Chain ID matches. - Symbol
     * matches. - Token ID matches. - Contract address (ignore case) matches.
     *
    ...
    private static final Comparator<BlockchainToken> blockchainTokenComparatorPerGasOrBatType =
            (token1, token2) -> {
                ...
                if (isNativeToken1 && !isNativeToken2) return -1;
                else if (!isNativeToken1 && isNativeToken2) return 1;
                else if (isBatToken1 && !isBatToken2) return -1;
                else if (!isBatToken1 && isBatToken2) return 1;
                else return token1.symbol.compareTo(token2.symbol);
            };
