...
const char16_t k_youtube_background_playback_script[] =
    ...
    u"    width: 50px;"
    u"    height: 50px;"
    u"    background-color: rgb(255, 238, 0);"
    u"    border: none;"
    u"    border-radius: 50%;"
    u"    cursor: pointer;"
    u"    z-index: 10000;"
    ...
    u"    box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5);"
    u"  `);"
    u"const iconElement = document.createElement('img');"
    u"iconElement.setAttribute('src', 'https://raw.githubusercontent.com/phosphor-icons/core/refs/heads/main/assets/bold/picture-in-picture-bold.svg');"
    u"iconElement.setAttribute('style', 'width: 24px; height: 24px;');"
    u"buttonElement.appendChild(iconElement);"
    ...
    u"        videoElement.requestPictureInPicture().catch(err => {"
    u"            console.error('Error entering PiP mode:', err);"
    u"        });"
    u"    }"
    u"});"
    u"let isDragging = false;"
    u"let offsetX, offsetY;"
    u"function enableDrag() {"
    u"    buttonElement.addEventListener('mousedown', (e) => {"
    u"        isDragging = true;"
    u"        offsetX = e.clientX - buttonElement.getBoundingClientRect().left;"
    u"        offsetY = e.clientY - buttonElement.getBoundingClientRect().top;"
    u"        document.addEventListener('mousemove', onMouseMove);"
    u"        document.addEventListener('mouseup', onMouseUp);"
    u"    });"
    u"}"
    u"function onMouseMove(e) {"
    u"    if (isDragging) {"
    u"        buttonElement.style.left = `${e.clientX - offsetX}px`;"
    u"        buttonElement.style.top = `${e.clientY - offsetY}px`;"
    u"    }"
    u"}"
    u"function onMouseUp() {"
    u"    isDragging = false;"
    u"    document.removeEventListener('mousemove', onMouseMove);"
    u"    document.removeEventListener('mouseup', onMouseUp);"
    u"}"
    u"enableDrag();"
    u"let touchStartY = 0;"
    u"let touchEndY = 0;"
    u"const videoElement = document.querySelector('video');"
    u"if (videoElement) {"
    u"    videoElement.addEventListener('touchstart', (e) => {"
    u"        touchStartY = e.changedTouches[0].screenY;"
    u"    });"
    u"    videoElement.addEventListener('touchend', (e) => {"
    u"        touchEndY = e.changedTouches[0].screenY;"
    u"        handleSwipeDownGesture();"
    u"    });"
    u"}"
    u"function handleSwipeDownGesture() {"
    u"    if (touchEndY > touchStartY) {"
    u"        const videoElement = document.querySelector('video');"
    u"        if (videoElement) {"
    u"            videoElement.removeAttribute('disablePictureInPicture');"
    u"            videoElement.requestPictureInPicture().catch(err => {"
    ...
    u"        'hidden': { value: false },"
    u"        'visibilityState': { value: 'visible' }"
    u"    });"
    u"}"
    u"window.addEventListener('visibilitychange', evt => evt.stopImmediatePropagation(), true);"
    u"if (IS_YOUTUBE) {"
    u"    loop(pressKey, 60000, 10000);"
    u"}"
    u"function pressKey() {"
    u"    const key = 18; // Alt key"
