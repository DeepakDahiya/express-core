...
async function applyPatches() {
  ...
  const coreRepoPath = config.expressCodeDir
...
const util = {
  ...
  updateBranding: () => {
    ...
    const braveComponentsDir = path.join(config.expressCodeDir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const braveAppDir = path.join(config.expressCodeDir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const braveBrowserResourcesDir = path.join(config.expressCodeDir, 'browser', 'resources')
    const braveAppVectorIconsDir = path.join(config.expressCodeDir, 'components')
    const chromeAndroidJavaStringsTranslationsDir = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'strings', 'translations')
    const braveAndroidJavaStringsTranslationsDir = path.join(config.expressCodeDir, 'browser', 'ui', 'android', 'strings', 'translations')
    ...
    const braveAndroidTabUiJavaStringsTranslationsDir = path.join(config.expressCodeDir, 'android', 'features', 'tab_ui', 'java', 'strings', 'translations')
    ...
    fileMap.add([path.join(config.expressCodeDir, 'ui', 'webui', 'resources', 'css', 'text_defaults_md.css'),
    ...
    fileMap.add([
      path.join(config.expressCodeDir, 'node_modules', '@brave', 'leo', 'icons',
    ...
    if (config.targetOS === 'android') {
      ...
      const androidTranslateResSource = path.join(config.expressCodeDir, 'components', 'translate','content' , 'android', 'java', 'res')
      ...
      const androidResSource = path.join(config.expressCodeDir, 'android', 'java', 'res')
      const androidResDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res')
      const androidResTemplateSource = path.join(config.expressCodeDir, 'android', 'java', 'res_template')
      const androidResTemplateDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_template')
      const androidContentPublicResSource = path.join(config.expressCodeDir, 'content', 'public', 'android', 'java', 'res')
      const androidContentPublicResDest = path.join(config.srcDir, 'content', 'public', 'android', 'java', 'res')
      const androidTouchtoFillResSource = path.join(config.expressCodeDir, 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidTouchtoFillResDest = path.join(config.srcDir, 'chrome', 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidToolbarResSource = path.join(config.expressCodeDir, 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidToolbarResDest = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidComponentsWidgetResSource = path.join(config.expressCodeDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidComponentsWidgetResDest = path.join(config.srcDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidComponentsStylesResSource = path.join(config.expressCodeDir, 'components', 'browser_ui', 'styles', 'android', 'java', 'res')
      const androidComponentsStylesResDest = path.join(config.srcDir, 'components', 'browser_ui', 'styles', 'android', 'java', 'res')
      const androidSafeBrowsingResSource = path.join(config.expressCodeDir, 'browser', 'safe_browsing', 'android', 'java', 'res')
      const androidSafeBrowsingResDest = path.join(config.srcDir, 'chrome', 'browser', 'safe_browsing', 'android', 'java', 'res')
      const androidDownloadInternalResSource = path.join(config.expressCodeDir, 'browser', 'download', 'internal', 'android', 'java', 'res')
      const androidDownloadInternalResDest = path.join(config.srcDir, 'chrome', 'browser', 'download', 'internal', 'android', 'java', 'res')
      const androidFeaturesTabUiResSource = path.join(config.expressCodeDir, 'android', 'features', 'tab_ui', 'java', 'res')
      const androidFeaturesTabUiDest = path.join(config.srcDir, 'chrome', 'android', 'features', 'tab_ui', 'java', 'res')
      const androidComponentsOmniboxResSource = path.join(config.expressCodeDir, 'components', 'omnibox', 'browser', 'android', 'java', 'res')
      const androidComponentsOmniboxResDest = path.join(config.srcDir, 'components', 'omnibox', 'browser', 'android', 'java', 'res')
      const androidBrowserUiOmniboxResSource = path.join(config.expressCodeDir, 'browser', 'ui', 'android', 'omnibox', 'java', 'brave_res')
  ...
  updateMidlFiles: () => {
    Log.progressScope('update midl files', () => {
      for (const source of ["google_update", "brave"]) {
        fs.copySync(
          path.join(config.expressCodeDir, 'win_build_output', 'midl', source),
  ...
  buildNativeRedirectCC: (options = config.defaultOptions) => {
    ...
    if (!config.isCI &&
        ...
        fs.statSync(path.join(config.expressCodeDir, 'tools', 'redirect_cc', 'redirect_cc.cc')).mtime) {
      return
    }
  ...
  generateXcodeWorkspace: (options = config.defaultOptions) => {
    ...
    const genScript = path.join(config.expressCodeDir, 'vendor', 'gn-project-generators', 'xcode.py')
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.expressCodeDir
    ...
    util.run(
        ...
        [
          ...
          path.join(
              config.expressCodeDir, 'build', 'commands', 'scripts', 'lint.py'),
  ...
  presubmit: (options = {}) => {
    ...
    util.runGit(
        config.expressCodeDir, ['config', '--unset-all', 'gerrit.host'], true)
    let cmd_options = config.defaultOptions
    cmd_options.cwd = config.expressCodeDir
    cmd_options = mergeWithDefault(cmd_options)
    cmd = 'git'
